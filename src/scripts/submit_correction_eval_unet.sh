...
ROLLOUT=${1:-2000}
SIM_FILE=${2:-sim8.h5}
OUT_DIR=${3:-$SCRATCH/correction_eval_unet_results}
# Same defaults as submit_correction_eval.sh, aligned with scale_separation.ipynb
# (diffusion16modes_RE90) rather than correction_eval_unet.py's own 7/3 defaults.
S_INIT=${S_INIT:-1}
S_STOP=${S_STOP:-0}
SEED=${SEED:-101}
...
python evaluation/correction_eval_unet.py \
    --data_dir         "$DATA_DIR" \
    --runs_dir         "$RUNS_DIR" \
