import os
from training.dataset_manager import DatasetManagerMultiRe
from training.trainer_diffusion import TrainerDiffusionParametric
from training.factory import Factory
from training.diffusion_model_factory import build_diffusion_backbone
...
class DiffusionTrainingParametric():
    """DiffusionTraining (cf. diffusion_training.py) for a Re-conditioned
    corrector: trains one of several backbones (config's "model_type", cf.
    training/diffusion_model_factory.py -- fno_concat, fno_hyper, unet_film,
    unet_concat) across several Reynolds numbers (config's re_values) on
    Parameterized_Neural_Operator's kolmogorov_parametric dataset, instead of
    one fixed-Re model per Re. Everything else (directory layout,
    checkpointing, optimizer/scheduler wiring) mirrors DiffusionTraining
    exactly -- the model_type dispatch is the only thing that varies, so one
    training class covers all four variants instead of near-duplicating this
    file four times."""
    ...
    def __init__(self, args):
        ...
        self.re_values = self.args["re_values"]
        ...
        self.datasets = DatasetManagerMultiRe(data_rep=DATA_DIR, exp_dir=self.exp_dir,
                                              re_values=self.re_values, seq_length=self.seq_length,
                                              batch_size=self.batch_size, num_workers=self.num_workers,
                                              stride=self.stride, ds=self.ds, normalize=self.normalize)
        print("Datasets loaded.")
        print("Dataset summary:")
        print(f"Training samples: {self.datasets.n_train}")
        print(f"Re values: {self.re_values}  (param_mean={self.datasets.param_mean:.4f}, "
              f"param_std={self.datasets.param_std:.4f})")
        if self.normalize:
            print(f"Field normalization enabled: x_mean={self.datasets.x_mean.tolist()}, "
                  f"x_std={self.datasets.x_std.tolist()}")

        self.name_weights_to_load = self.args.get("name_weights_to_load", None)

        # Model-specific fields (k_max_x, width, depth, base_width, n_basis, ...)
        # are read directly from self.args by build_diffusion_backbone() based
        # on model_type, not unpacked into instance attributes here.
    ...
    def execute_experience(self):
        ...
        model = build_diffusion_backbone(self.args, self.datasets.param_mean,
                                         self.datasets.param_std, self.device)
        ...
        diffusion_model = Diffusion(model=model,
                                    ...
                                    x_mean=self.datasets.x_mean,
                                    x_std=self.datasets.x_std)
        ...
        trainer = TrainerDiffusionParametric(model=diffusion_model,
