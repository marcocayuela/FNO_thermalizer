...
class HFNO_2D(nn.Module):
    def __init__(self,modes, depth, width_MLP, n_layers_MLP, input_size, output_size, res, residual=None, mode_separation="L2", device="cpu"):
        ...
        # Lifting transformation, corresponding to a simple linear transformation
        self.P = nn.Linear(self.input_size, self.depth, device=self.device)  # input channel is 2: (u(x,y),v(x,y))
        ...
        if self.mode_separation=="L2":
            self.k_magnitude = np.sqrt(kx**2 + ky**2) 
        elif self.mode_separation=="Linf":
    ...
    def forward(self, x, indexes=None):
        ...
        for i in range(len(self.modes)):
            ...
            output_pad = torch.zeros(sh[0],self.output_size, self.res, self.res//2+1, dtype=torch.cfloat, device=self.device)
        ...
        if indexes.shape[0]==0:
            z = torch.zeros_like(torch.fft.irfft2(output_pad,s=(mx, mz), device=self.device))
            z = z.permute(0,2,3,1)
        else:
            z = torch.sum(torch.stack([fourier_parts[i] for i in indexes]), axis=0)

        if self.residual is not None:
            ft_w_part = torch.fft.rfft2(x)
            ft_w_part[:,:,self.mask_res] = 0
            x_w_part = torch.fft.irfft2(ft_w_part, s=(mx, mz))
            linear_part = self.W(x_w_part).permute(0, 2, 3, 1)
            z += linear_part
